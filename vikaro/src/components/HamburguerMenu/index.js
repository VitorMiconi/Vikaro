  ...
  return (
    <>  
      ...
      <nav className={`divide-y ${open ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-linear absolute text-start bg-black h-screen w-screen left-0 top-0 py-16 px-6`}>
        <a href="#projects" onClick={handleLinkClick} className="py-6 text-white font-montserrat font-light block">
          PROJETOS
        </a>
        <a href="#services" onClick={handleLinkClick} className="py-6 text-white font-lemon font-montserrat block">
          SERVIÇOS
        </a>
        <a href="#about" onClick={handleLinkClick} className="py-6 text-white font-lemon font-montserrat block">
          SOBRE
        </a>
      </nav>
    </>
